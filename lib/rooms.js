...
function getPayloadFileFilename(value) {
  const match = value.match(/\/api\/media\/file\/([^/?#]+)/);
...
function normalizeStoredMediaURL(value) {
  ...
  const payloadFileFilename = getPayloadFileFilename(url);

  if (payloadFileFilename) {
    return getSupabaseMediaURL(payloadFileFilename);
  }
  ...
  if (url.startsWith("/media/")) {
    return getSupabaseMediaURL(getFilenameFromPath(url)) || (isVercelRuntime ? "" : url);
  }
  ...
  return getSupabaseMediaURL(getFilenameFromPath(url)) || (isVercelRuntime ? "" : `/media/${url}`);
}
